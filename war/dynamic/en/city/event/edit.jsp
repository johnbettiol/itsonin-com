						...
						<div class="panel-body">
							...
							<div class="text-center">
								<c:if test="${event.startTime == null}"> 
									<span>Happening sometime</span>
								</c:if>
								<c:if test="${event.startTime lt now}"> 
									<span>Happening NOW</span>
								</c:if>
								<c:if test="${event.startTime gt now}">
									<div>
										<fmt:formatDate pattern="yyyy-MM-dd" value="${event.startTime}" var="formattedStartTime"/>
										<input size="16" type="text" id="startTime" name="startTime" value="${formattedStartTime}" readonly class="form_datetime">
										<span>to</span>
										<fmt:formatDate pattern="yyyy-MM-dd" value="${event.startTime}" var="formattedEndTime" />
										<input size="16" type="text" id="endTime" name="endTime" value="${formattedEndTime}" readonly class="form_datetime">
									</div>
								</c:if>
							</div>
						</div>
